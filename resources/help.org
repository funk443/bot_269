...
*預設前綴為a269, 某些伺服器可能有修改過前綴，可以使用( ~PREFIX~ )來查詢。*
*說明書中有and符號請忽略，這些是用來做分隔之用，指令中不會有這些符號。*

請使用 ~a269 help <目錄中的數字(如1, 2, 3等)>~ 來看更詳細的說明。

直接在聊天室中輸入 ~COPYRIGHT~ 來查看版權聲明。
直接在聊天室中輸入 ~LICENSE~ 來查看AGPLv3之內容。
直接在聊天室中輸入 ~LICENSE_zh~ 來查看AGPLv3之非官方中文譯本。

原始碼可以在 https://github.com/funk443/bot_269 找到。
html版使用說明請參閱https://ctoid.synology.me/bot_269.html
...
- 是否充許非管理員新增回覆或是回應
  用法: ~a269 svr_conf set allow_set_by_non_admin true/false~ 
...
- 清除twitch id清單
  用法: ~a269 svr_conf clr twitch_name~
...
- 刪除回覆(del)
  用法: ~a269 react del <觸發關鍵字>~ 
  用來刪除在資料庫中的回應。  


- 尋找關鍵字(findk)
  用法: ~a269 react findk <回應表符>~ 
  用回應來反查關鍵字。  


- 尋找回覆(findv)
  用法: ~a269 react findv <觸發關鍵字>~ 
  用關鍵字來找回應。  
